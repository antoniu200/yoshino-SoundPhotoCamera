    ...
    public SoundPhotoSettingUi(SoundPhotoCameraActivity activity, SettingDialogStack dialogStack, CommonSettings commonSettings, SettingPreferenceManager preference, SettingChangerInterface<ParameterValue> settingChanger, SoundPhotoViewFinder viewFiner) {
        ...
        this.mShortcutItemExecutor = new SettingExecutorInterface<Shortcut>() { // from class: com.sonymobile.android.addoncamera.soundphoto.view.settingui.SoundPhotoSettingUi.1
            ...
            public void onExecute(TypedSettingItem<Shortcut> item) {
                Shortcut tag = item.getData();
                switch (AnonymousClass2.$SwitchMap$com$sonymobile$android$addoncamera$soundphoto$view$settingui$SoundPhotoSettingUi$Shortcut[tag.ordinal()]) {
                    case 2:
                        if (SoundPhotoSettingUi.this.mDialogStack.isOpened(tag)) {
                            SoundPhotoSettingUi.this.mDialogStack.closeDialogs();
                            break;
                        } else {
                            SoundPhotoSettingUi.this.mDialogStack.closeDialogs();
                            SoundPhotoSettingUi.this.mDialogStack.updateShortcutSelected(tag);
                            SettingAdapter flashItems = SoundPhotoSettingUi.this.mItemResolver.generateFlashItems();
                            if (!flashItems.isEmpty()) {
                                SoundPhotoSettingUi.this.openShortcutDialog(flashItems, R.string.cam_strings_flash_torch_txt, tag);
                                break;
                            }
                        }
                        break;
                    case 3:
                        if (SoundPhotoSettingUi.this.mActivity.isSelfTimerAvailable()) {
                            if (SoundPhotoSettingUi.this.mDialogStack.isOpened(tag)) {
                                SoundPhotoSettingUi.this.mDialogStack.closeDialogs();
                                break;
                            } else {
                                SoundPhotoSettingUi.this.mDialogStack.closeDialogs();
                                SoundPhotoSettingUi.this.mDialogStack.updateShortcutSelected(tag);
                                SettingAdapter selftimerItems = SoundPhotoSettingUi.this.mItemResolver.generateSelfTimerItems();
                                if (!selftimerItems.isEmpty()) {
                                    SoundPhotoSettingUi.this.openShortcutDialog(selftimerItems, R.string.cam_strings_self_timer_txt, tag);
                                    break;
                                }
                            }
                        }
                        break;
                    case 4:
                        if (SoundPhotoSettingUi.this.mActivity.isMenuAvailable()) {
                            if (SoundPhotoSettingUi.this.mDialogStack.isOpened(tag)) {
                                SoundPhotoSettingUi.this.mDialogStack.closeDialogs();
                                break;
                            } else {
                                SoundPhotoSettingUi.this.openMainSettingsDialog(tag);
                                break;
                            }
                        } else {
                            SoundPhotoSettingUi.this.mDialogStack.clearShortcutSelected();
                            break;
                        }
                    case 5:
                        SoundPhotoSettingUi.this.closeDialogs(false);
                        SoundPhotoSettingUi.this.mViewFinder.onToggledCameraSwitch();
                        break;
                }
            }
        };
