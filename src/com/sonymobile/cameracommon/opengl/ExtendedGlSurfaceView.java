...
public class ExtendedGlSurfaceView extends GLSurfaceView {
    ...
    static String loadShaderSourceCodesFrom(Context context, int[] glslFileId) {
        ...
        int i = 0;
        try {
            while (i < glslFileId.length) {
                Resources res = context.getResources();
                br = new BufferedReader(new InputStreamReader(res.openRawResource(glslFileId[i])));

                String line = br.readLine();
                while (line != null) {
                    builder.append(line);           // smali does not append '\n'
                    line = br.readLine();
                }
                ...
                if (br != null) {
                    try {
                        ...
                        // This close happens in the normal (non-exception) path in smali via a catch label,
                        // so we mirror the same error message.
