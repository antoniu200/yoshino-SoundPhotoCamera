...
public class Notification extends RelativeLayout {
    ...
    public void onMacroStatusChanged(boolean show, CameraExtension.SceneMode sceneMode) {
        if (show) {
    ...
    public void onRecognisedSceneChanged(RecognizedScene scene) {
        this.mScene = CameraExtension.SceneMode.AUTO;
        int iconId = scene.getIconId();
        int textId = scene.getTextId();
        if (iconId > 0 && textId > 0) {
            ...
            if (this.mSensorOrientation == 2) {
                ...
                this.mScene = scene.getSceneMode();
            }
        } else {
            findViewById(R.id.recognised_scene).setVisibility(4);
        }
    ...
    public void onRecognisedConditionChanged(RecognizedCondition condition) {
        int iconId = condition.getIconId();
        int textId = condition.getTextId();
        findViewById(R.id.recognised_condition).setVisibility(0);
        if (iconId > 0) {
    ...
    public void setSensorOrientation(int orientation) {
        ...
        this.mSceneIcon.setRotation(degree);
        setOrientationSceneText();
        this.mConditionIcon.setRotation(degree);
